...
struct BudgetView: View {
    ...
    var categoryBreakdown: [(String, String, Double, Color)] {
        let cats: [(String, String, Color)] = [
            ("Food", "Essen", .orange),
            ("Transport", "Transport", .red),
            ("Accommodation", "Unterkunft", .purple),
            ("Activities", "Aktivitäten", .green),
            ("General", "Sonstiges", .blue)
        ]
        return cats.compactMap { key, label, color in
            let amount = tripExpenses.filter { $0.category == key }.reduce(0) { $0 + $1.amount }
            return amount > 0 ? (key, label, amount, color) : nil
        }
    }
