...
struct AgreementsView: View {
    ...
    func filterColor(for filter: String) -> Color {
        switch filter {
        case "Offen": return .blue
        case "Beschlossen": return .green
        case "Erledigt": return .gray
                ...
                HStack(spacing: 8) {
                    ForEach(filters, id: \.self) { filter in
                        ...
                        Button(action: { withAnimation { selectedFilter = filter } }) {
                            HStack(spacing: 4) {
                                ...
                                if count > 0 && filter != "Alle" {
                                    Text("\(count)")
                                        .font(.caption2)
                                        .fontWeight(.bold)
                                        .foregroundColor(selectedFilter == filter ? filterColor(for: filter) : .white)
                            ...
                            .padding(.vertical, 8)
                            .background(selectedFilter == filter ? filterColor(for: filter) : Color(.systemGray6))
            ...
            if tripAgreements.isEmpty {
                VStack(spacing: 16) {
                    Image(systemName: "checkmark.seal.fill")
                        .font(.system(size: 56))
                        .foregroundColor(currentAccentColor.opacity(0.3))
                ...
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ForEach(tripAgreements, id: \.id) { agreement in
                            NavigationLink(destination: AgreementDetailView(agreement: agreement, trip: trip, accentColor: currentAccentColor)) {
                                AgreementCardView(agreement: agreement)
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 12)
                                            .stroke(currentAccentColor.opacity(0.12), lineWidth: 1)
                                    )
                            }
                            .buttonStyle(.plain)
                        }
