...
struct TripDetailView: View {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 10) {
                    ForEach(menuItems) { item in
                        NavigationLink(destination: destinationView(for: item.destination, accentColor: item.color)) {
                            HStack(spacing: 14) {
    ...
    @ViewBuilder
    func destinationView(for dest: TripMenuDestination, accentColor: Color) -> some View {
        switch dest {
        case .ideensammlung:
            VotingOverviewView(trip: trip, accentColor: accentColor)
        case .packliste:
        ...
        case .chat:
            ChatView(trip: trip)
        case .absprachen:
            AgreementsView(trip: trip, accentColor: accentColor)
        case .gruppe:
